...
# row 4, all elements
matrix1[ , 3] # column 3, all elements 
matrix1[c(12, 14)] #
matrix1[c(12:14)] #
matrix1[2:4, 1:3] #
...
e <- c("red", "white", "red", NA) # What type of vector? categorical vector 
...
```{r DataExercise-1a}
high_temperatures <- c(83, 83, 84, 78, 71, 79, 82, 85, 84, 82)
low_temperatures <- c(56, 58, 60, 61, 63, 63, 64, 64, 63, 60)
...
```{r DataExercise-1b}
high_temp_celcius <- ((high_temperatures-32)*(5/9))
low_temp_celcius <- ((low_temperatures-32)*(5/9))
...
```{r DataExercise-1c}
ten_day_forcast<-data.frame(high_temperatures,high_temp_celcius,low_temperatures,low_temp_celcius)

colnames(ten_day_forcast)<-c("High Temperatures (Fahrenheit)", "High Temperatures (Celcius)", "Low Temperatures (Fahrenheit)", "Low Temperatures (Celcius)")

View(ten_day_forcast)

# you can also do this in one step - name when you define the data frame 
```
...
```{r DataExercise-1d}
summary(ten_day_forcast)

# cannot use sd on a data frame, use it for an element in your data frame - sd is standard deviation
sd(high_temperatures)
```
