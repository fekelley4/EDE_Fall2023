...
dim(NTL.phys.data_1999)

# Exercise 2: 
# filter NTL.phys.data for Tuesday Lake from 1990 through 1999.
summary(NTL.phys.data$lakename)
NTL.phys.data_TuesLake <- filter(NTL.phys.data, lakename %in% c("Tuesday Lake"))
NTL.phys.data_TuesLake_1990through1999 <- filter(NTL.phys.data_TuesLake, year4 %in% c(1990:1999))
NTL.phys.data_TuesLake_1990through1999
```
...
month1990thru1999 <- month(NTL.phys.data_TuesLake_1990through1999$sampledate)
#Exercise 3: Using a pipe filter NTL.phys.data for Tuesday Lake from 1990 
# through 1999 only for July.
NTL.phys.data_TuesLake_1990thru1999_July <- NTL.phys.data %>%
  filter(lakename == "Tuesday Lake") %>% 
  filter(year4 %in% c(1990:1999)) %>% 
  filter(daynum %in% c(181:212))
#daynum manually went into table and extracted the day numbers 

NTL.phys.data_TuesLake_1990thru1999_July
