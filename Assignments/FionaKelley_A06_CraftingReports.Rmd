...
#Create metadata inputs for the table. 
metadata_titles <- c("Source", "Date", "Filename")
metadata_info <- c("EPA Air Quality System (AQS)", "2018-2019", 
                   "EPAair_O3_PM25_NC1819_Processed.csv")
metadata_DataFrame <- data.frame("Item" = metadata_titles, 
                                 "Value" = metadata_info)

#Creating a table for the metadata information. 
knitr::kable(metadata_DataFrame, caption = "EPA Air Quality", align = "rl")
...
```{r 2018 plot, echo=FALSE, fig.height = 4, fig.align='left'}
#Loading package to isolate years. 
library(dplyr)

#Plotting 2018 ozone levels by month. 
ozone18_plot <- ggplot(
  subset(epa_data, year(Date) == 2018),
  aes(x = month.abb[month(Date)], y = Ozone)) + geom_boxplot(na.rm = TRUE) +
  scale_x_discrete(limits = month.abb) +
  labs(x = "Month", y = "Ozone", title = "2018 Ozone Levels by Month")
ozone18_plot


```

```{r 2019 plot, echo=FALSE, fig.height = 4, fig.align='left'}
#Plotting 2019 ozone levels by month. 
ozone19_plot <- ggplot(
  subset(epa_data, year(Date) == 2019),
  aes(x = month.abb[month(Date)], y = Ozone)) + geom_boxplot(na.rm = TRUE) +
  scale_x_discrete(limits = month.abb) +
  labs(x = "Month", y = "Ozone", title = "2019 Ozone Levels by Month")
...
## Git Repository
https://github.com/fekelley4/EDE_Fall2023
